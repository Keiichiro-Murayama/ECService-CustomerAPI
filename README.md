...
| 4   | ログインユーザー情報取得API | GET      | /api/customer/profile                | 購入確認画面 (FP009)              | 必須 |
...
| 11  | 注文詳細取得API             | GET      | /api/customer/orders/{orderUuid}     | 購入履歴詳細画面 (FP012)          | 必須 |
...
ログインユーザーの過去の注文履歴一覧 (FP011) を取得します。注文日時の新しい順（降順）で返却されます。
...
顧客ログイン API から返却されたトークンおよび顧客名は、React の状態管理（例: useContext や Redux）を用いてアプリ全体で保持し、「ようこそ、〇〇さん」の画面表示や各 API の認可制御に利用します。
